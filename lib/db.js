...
// Ensure MongoDB Atlas SRV resolution works reliably across all ISP DNS
try {
...
// Global memory store for fast response and fallback
  ...
  try {
    if (mongoose.connection.readyState === 1 && mongoose.connection.db) {
      ...
      if (!hasDocs) {
        ...
        console.log('🍃 Loaded collections from MongoDB Atlas into Store');
      }
    } else {
      ensureSeedData();
    }
...
export const connectDB = async () => {
  ...
  try {
    ...
    console.log(`🍃 Connected to MongoDB: ${mongoose.connection.name || 'Atlas'}`);
    await loadFromMongoDB();
    hasLoadedData = true;
  } catch (error) {
    console.warn('MongoDB Atlas connection note (using in-memory data store):', error.message);
    ensureSeedData();
  } finally {
    isConnecting = false;
  }
